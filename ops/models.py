...
class TSN(nn.Module):
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            ...
            if self.is_shift:
                ...
                from ops.global_shift import make_global_shift

                make_global_shift(self.base_model, self.num_segments,
    ...
    def forward(self, input, no_reshape=False):
        ...
        input = torch.cat([x1, x2 - x1, x3 - x2, x4 - x3, x5 - x4], 1).view(-1, 10, x2.size()[2], x2.size()[3])

        if not no_reshape:
            sample_len = (3 if self.modality == "RGB" else 2) * self.new_length

            if self.modality == 'RGBDiff':#wu
                sample_len = 3 * self.new_length
                input = self._get_diff(input)

            #print(input.contiguous().view((-1, sample_len) + input.size()[-2:]).size())

            base_out1 = self.base_model(input.contiguous().view((-1, sample_len) + input.size()[-2:]))



        else:
            base_out = self.base_model(input)#(meiyou)

        if self.dropout > 0:#有


            base_out1 = self.new_fc(base_out1)  # linearceng
        if not self.before_softmax:#meiyou
        ...
        if self.reshape:
            if self.is_shift and self.temporal_pool:
                ...
                base_out1 = base_out1.view((-1, self.num_segments) + base_out1.size()[1:])

            output1 = self.consensus(base_out1)
